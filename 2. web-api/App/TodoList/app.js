...
function makeNewTodoNode(newTodo) {
  ...
  // 클래스 이름을 두 개 이상 add할 때는 각각 지정해야 한다.
  // 한 번에 공백 포함 두 개 이상 설정하면 에러남.
...
function changeCheckState($label) {
  /*
    ...
    checked라는 클래스 이름을 추가하세요. 근데, 할 일 완료는 껐다 켰다 할 수 있어야 합니다.
    -> 클래스 이름을 뗏다 붙였다 할 수 있어야 합니다.
  */
...
function removeTodoDate($delTarget) {
  ...
  // 시간 지연은 1.5초 진행.(시간을 지연하는 window 함수가 있음)
  setTimeout(() => {
    document.querySelector('.todo-list').removeChild($delTarget);
  }, 1000);

  // 배열 내에 있는 객체도 삭제를 진행
  // 삭제되는 객체가 배열 안의 몇 번째인지를 확인 -> 할 일 완료 처리 함수쪽에 비슷한 로직이 있습니다.
  // 함수화 시켜보세요.
...
function enterModifyMode($modSpan) {
  ...
  // input 태그에는 .modify-input을 추가하고, input에는 기존의 할 일 텍스트가 미리 작성되어 있어야 합니다.
...
function modifyTodoData($modCompleteSpan) {
  ...
  // 배열 내의 id가 일치하는 객체를 찾아서 text 프로퍼티의 값을 수정된 값으로 변경해 주어야 합니다.
...
(function () {
  ...
  $todoList.addEventListener('click', e => {
    ...
    removeTodoDate(e.target.parentNode.parentNode); // 이벤트가 발생한 곳이 조상을 매개값으로 전달 (li)
  });
  ...
  $todoList.addEventListener('click', e => {
    if (e.target.matches('.todo-list .modify span.lnr-undo')) {
      ...
      modifyTodoData(e.target); // 수정 모드에서 수정을 확정지으려는 이벤트
    } else {
      return;
    }
  });
